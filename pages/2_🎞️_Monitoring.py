...
def main(tmp_save, tmp_save_cvt, tmp_result, tmp_result_cvt):
    ...
    if video_data:
        ...
        subprocess.call(args=f"ffmpeg -y -i {tmp_save} -c:v libx264 {tmp_save_cvt}".split(" "))
        video_info = get_video_info(tmp_save_cvt)
        st.write(video_info[1:])
    
        if st.button('분석 시작'):
            detect_video( model, video_info, tmp_result)
            subprocess.call(args=f"ffmpeg -y -i {tmp_result} -c:v libx264 {tmp_result_cvt}".split(" "))
